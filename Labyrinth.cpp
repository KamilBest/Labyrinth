	...
	do {
		...
		if ((board[8][2] == FLD_PLAYER) && (quest == false)) {
			...
			cout << "Rano chodzi na czterech łapach," << endl
					<< "wieczorem na trzech," << endl << "A w południe na dwóch?"
					<< endl;
			cout << "a) pies," << endl << "b) niedźwiedź," << endl << "c) człowiek,"
					<< endl << "d) inne zwierze" << endl;
			do{
			...
			else if((answer!='a')and(answer!='b')and(answer!='c')and(answer!='d'))
...
int main() {
	...
	case 1:
		...
		break;
	case 2:
		cout<<"Aby ukończyć labirynt musisz pokonać wszystkie przeszkody \noraz zebrać gwiazdkę."<<endl;
		break;
	case 0:
		exit(0);
		break;
